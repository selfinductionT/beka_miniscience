...
public:
  ...
  void move(double tau, double step) {
        // x += vx * tau;
        // y += vy * tau;
        // z += vz * tau;
        y += 2*z * tau;
        z += -2*y * tau;
	double phi = 2*tau*step;
	double y0 = y*cos(phi) - z*sin(phi);
	smth = sin(0.2*step - 0.25*sqrt((pow(x, 2)+pow(y0, 2))));
    }
};
...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
	    ...
	    double smth = sin(-0.25*sqrt((pow(pointX, 2) + pow(pointY, 2))));
	    double vx = 0;
	    double vy = 2*pointZ;
	    double vz = -2*pointY;
  ...
  void doTimeStep(double tau, double step) {
    ...
    void snapshot(unsigned int snap_number) {
        ...
        string fileName = "1-step-" + std::to_string(snap_number) + ".vtu";
